  ...
  const resolvers = {
    Query: {
      access: async (root, args, context) => {
        ...
        try {
          const response = await fetch(
            `https://cloud.ouraring.com/oauth/token?grant_type=authorization_code&code=${args.code}&client_id=${process.env.CLIENT_ID}&client_secret=${process.env.CLIENT_SECRET}`,
            {
              method: 'POST'
            }
          )
      ...
      getData: async (root, args, context) => {
        ...
        try {
          const response = await fetch(
            `https://api.ouraring.com/v1/${args.type}?start=${args.start}&end=${args.end}`,
            {
              method: 'GET',
